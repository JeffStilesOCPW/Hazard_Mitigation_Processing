...
#workbook = xlsxwriter.Workbook('F:\PROJECTS\Jeff\Hazard_Mitigation_Planning\Output_Data_2020\HazardSummary.xlsx')
workbook = xlsxwriter.Workbook('F:\PROJECTS\Jeff\Hazard_Mitigation_Planning\Output_Data_2020\HazardSummaryUnincorporated.xlsx')



### Landuse Value Lists SCAG Landuse Codes ###
## All Residential ##
resiLanduse = [1110,1111,1112,1113,1120,1121,1122,1123]


## All Other ##
otherLanduse = [1210,1211,1212,1213,1200,1220,1221,1222,1223,1230,1231,1232,1233,1240,1241,
                1242,1243,1244,1245,1246,1247,1250,1251,1252,1253,1260,1261,1262,1263,1264,1265,1266,1270,1271,1272,1273,
                1274,1275,1276,1300,1310,1311,1312,1313,1314,1320,1321,1322,1323,1324,1325,1330,1331,1332,1340,1400,1410,
                1411,1412,1413,1414,1415,1416,1417,1418,1420,1430,1431,1432,1433,1434,1435,1436,1437,1438,1440,1441,1442,
                1450,1460,1500,1600,1610,1620,1800,1810,1820,1830,1840,1850,1860,1870,1880,1890,2000,2100,2110,2120,2200,
                2300,2400,2500,2600,2700,3000,3100,3200,3300,3400,1900,4000,4100,4200,4300,4400,4500,7777,1700,8888,9999]
...
# ## Define hazard by land use variables, define set to zero and start counting ##
OtherLU100 = OtherLU100p = OtherLU500 = OtherLULandSlide = 0
OtherLUFire = OtherLUEQ = OtherLUPrado = OtherLUmhhw = OtherLUCoastFlood = 0
[OtherLU100,OtherLU100p,OtherLU500,
 OtherLULandSlide,OtherLUFire,OtherLUEQ,
 OtherLUPrado,OtherLUmhhw,OtherLUCoastFlood] = calcCountHazard(hazByLuDict,otherLanduse,OtherLU100,OtherLU100p,
              OtherLU500,OtherLULandSlide,OtherLUFire,OtherLUEQ,OtherLUPrado,OtherLUmhhw,OtherLUCoastFlood)


MixResi100 = MixResi100p = MixResi500 = MixResiLandSlide = 0
MixResiFire = MixResiEQ = MixResiPrado = MixResimhhw = MixResiCoastFlood = 0
[MixResi100,MixResi100p,MixResi500,
 MixResiLandSlide,MixResiFire,MixResiEQ,
 MixResiPrado,MixResimhhw,MixResiCoastFlood] = calcCountHazard(hazByLuDict,resiLanduse,MixResi100,MixResi100p,
              MixResi500,MixResiLandSlide,MixResiFire,MixResiEQ,MixResiPrado,MixResimhhw,MixResiCoastFlood)
...
## Sum of population by block group and hazard ##
TotPop100 = TotPop100p = TotPop500 = TotPopLandSlide = 0
TotPopFire = TotPopEQ = TotPopPrado = TotPopmhhw = TotPopCoastFlood = 0

## Proceed through variables and sum population once for each geoid value on parecls that intersect hazards ##
alreadyAdded100yr = []
alreadyAddedtsunami = []
alreadyAdded500yr = []
alreadyAddedlandslide = []
alreadyAddedfire = []
alreadyAddedeq = []
alreadyAddedPrado = [] 
alreadyAddedmhhw = []
alreadyAddedCoastFlood = []
...
### Save export to formatted xlsx spreadsheet ###
worksheet = workbook.add_worksheet()
hazardMatrix = [
    ['Landuse','100 yr Flood','Tsunami Innundation','500 yr flood','Landslide','Fire','Earthquake','Prado Inundation','MHHW','Coast Flood'],
    
    ['All Other Parcels Within Hazard',
     OtherLU100,OtherLU100p,OtherLU500,OtherLULandSlide,OtherLUFire,OtherLUEQ,OtherLUPrado,OtherLUmhhw,OtherLUCoastFlood],
    
    ['Residential Parcels Within Hazard',
     MixResi100,MixResi100p,MixResi500,MixResiLandSlide,MixResiFire,MixResiEQ,MixResiPrado,MixResimhhw,MixResiCoastFlood],

    ['All Parcels Within Hazard',
     TotParcel100,TotParcel100p,TotParcel500,TotParcelLandSlide,TotParcelFire,TotParcelEQ,TotParcelPrado,TotParcelmhhw,TotParcelCoastFlood],
    
    ['All Population Within Hazard',
     TotPop100,TotPop100p,TotPop500,TotPopLandSlide,TotPopFire,TotPopEQ,TotPopPrado,TotPopmhhw,TotPopCoastFlood],
]
print(hazardMatrix)
